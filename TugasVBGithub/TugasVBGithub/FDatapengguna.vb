...
Public Class FDatapengguna
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If (tempEmail <> Nothing) Then 'lagi edit
            If pass = Nothing Then 'kalo ga masukin password baru
                sql = "update `User` set tipe = '" & ComboBox1.Text() & "',  " & _
                        "aktif = " & CheckBox1.CheckState & " " & _
                        "where username = '" & tempEmail & "'"
            Else
                sql = "update `User` set tipe = '" & ComboBox1.Text() & "',  " & _
                         "pwd = '" & pass & "', " & _
                         "aktif = " & CheckBox1.CheckState & " " & _
            ...
            End If
        Else
            sql = "insert into `User` (username, pwd, tipe, aktif) " & _
    ...
    Async Function loadGrid(ByVal cari As String) As Task
        ...
        If cari = Nothing Then
            sql = "select username, tipe, aktif from `User`"
        Else
            sql = "select username, tipe, aktif from `User` " & _
